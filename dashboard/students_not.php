                      ...
                      <div class="table-responsive">
                        <table id="studentDetails" class="display expandable-table" style="width:100%">
                          <thead>
                            ...
                            <?php
                            $queryStudents = "SELECT u.`user_id`,`name`,`reg_date`,`email`,`telephone_number` FROM `user` u, `user_detail` d , `role` r, `referral` f WHERE d.`user_id` =u.`user_id` AND f.`user_id` =d.`user_id` AND u.`role_id` =r.`role_id` AND r.`role_name` = 'Student' AND d.network_referral_id IS NULL";
                            if ($accountRole != 'Admin') {
                              $referral_idQ = "SELECT `referral_id` AS f FROM `referral` WHERE `user_id` = '$login_user_id' ";
                              $referralID = $db->getValueAsf($referral_idQ);
                              $queryStudents = "SELECT u.`user_id`,`name`,`reg_date`,`email`,`telephone_number` FROM `user` u, `user_detail` d , `role` r, `referral` f WHERE d.`user_id` =u.`user_id` AND f.`referral_id` =d.`referral_id` AND u.`role_id` =r.`role_id` AND f.`referral_id` = '$referralID' AND d.network_referral_id IS NULL";
